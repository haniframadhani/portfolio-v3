<script setup lang="ts">
  ...
  async function onSubmit(event: FormSubmitEvent<Schema>) {
    try {
      ...
      const thumbnailUrl = await uploadThumbnail("blogs", thumbnail);
      await insertIntoTable("blogs", { title, excerpt, content, thumbnail: thumbnailUrl });
      router.push("/dashboard/blogs");
      toast.add({ title: "Success create blog", color: "success" });
    } catch (error) {
